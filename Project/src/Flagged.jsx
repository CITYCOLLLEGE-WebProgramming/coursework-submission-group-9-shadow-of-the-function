...
const Flagged = () => {
  ...
  useEffect(() => {
    ...
    fetchFlaggedMessages(sortCriteria, sortOrder);
  ...
  const fetchFlaggedMessages = (criteria, order) => {
    axios.get(`http://127.0.0.1:3000/flagged-messages?sort=${criteria}&order=${order}`, { withCredentials: true })
      .then(response => {
  ...
  const handleStarClick = (event, message) => {
    ...
    axios.post('http://127.0.0.1:3000/flag-message', { messageId: message._id }, { withCredentials: true })
      .then(response => {
        console.log('Flag status updated:', response.data);
        fetchFlaggedMessages(sortCriteria, sortOrder); // Refresh flagged messages
      })
      .catch(error => {
  ...
  const handleDeleteClick = (event, message) => {
    event.stopPropagation();
    axios
      .delete(`http://127.0.0.1:3000/delete-message/${message._id}`, {
        withCredentials: true,
      })
      .then((response) => {
  ...
  const handleCheckboxClick = (event, message) => {
    event.stopPropagation();
                ...
                <div className="email-header">
                  <div className="email-actions" onClick={(e) => e.stopPropagation()}>
                    <button className="star-btn" onClick={(e) => handleStarClick(e, message)}>
                      <i className={`fa-solid fa-star${message.flagged ? '' : '-o'}`}></i>
                    </button>
                    <input type="checkbox" className="select-checkbox" onClick={(e) => handleCheckboxClick(e, message)}/>
                  </div>
